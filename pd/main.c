...
void Rubberband_fillRect(SDL_Renderer *r, int x0, int y0, int x1, int y1, s)
{
	int t;
...
int main(void)
{
	...
	while (!quit)
	{
		...
		while (SDL_PollEvent(&event))
		{
			...
			case SDL_FINGERDOWN:
				fingerDown = true;
				fingerX = event.tfinger.x * sw;
				fingerY = event.tfinger.y * sh;
				break;
		...
		if (drawButton(renderer, 4, gap, gap + 32 + us, us, us, 0))
		{
			for (int i = 0; i < bw * bh; i++)
				bb[i] = bp[bc * bw * bh + i];
		};

		if (drawButton(renderer, 5, sw - us - gap, gap + 32 + us, us, us, 0))
		{
			if (bn < bc)
			{
				bn = bc;
		...
		if (drawButton(renderer, 6, (sw - us) / 2, gap + 32 + us, us, us, 0))
		{
			for (int i = 0; i < bw * bh; i++)
			{
				if (bc == bn)
		...
		if (drawButton(renderer, 7, gap, (gap + 32 + us) * 2, us, us, 0))
		{
			for (int i = 0; i <= bn; i++)
			{
		...
		if (drawButton(renderer, 8, sw - us - gap, (gap + 32 + us) * 2, us, us, 0))
		{
			free(bp);
			bp = NULL;
			bc = 0;
		...
		if (drawButton(renderer, 9, (sw - us) / 4, (gap + 32 + us) * 2, us, us, 0))
		{
			gameState = GAMESTATE_FLOODFILL;
		}

		if (drawButton(renderer, 10, (sw - us) / 4 * 3, (gap + 32 + us) * 2, us, us, 0))
		{
			gameState = GAMESTATE_DROPPER;
		}

		if (drawButton(renderer, 11, (sw - us) / 2, (gap + 32 + us) * 2, us, us, 0))
		{
			gameState = GAMESTATE_TARGET;
		}

		if (drawButton(renderer, 12, gap, (gap + 32 + us) * 3, us, us, 0))
		{
			gameState = GAMESTATE_LINE;
		}

		if (drawButton(renderer, 13, (sw - us) / 4, (gap + 32 + us) * 3, us, us, 0))
		{
			gameState = GAMESTATE_RECT;
		}

		if (drawButton(renderer, 14, (sw - us) / 4 * 2, (gap + 32 + us) * 3, us, us, 0))
		{
			gameState = GAMESTATE_FILLRECT;
		}

		if (drawButton(renderer, 15, (sw - us) / 4 * 3, (gap + 32 + us) * 3, us, us, 0))
		{
			gameState = GAMESTATE_OVAL;
		}

		if (drawButton(renderer, 16, sw - us - gap, (gap + 32 + us) * 3, us, us, 0))
		{
			gameState = GAMESTATE_FILLOVAL;
		}

		if (drawButton(renderer, 17, (sw - us) / 2, (gap + 32 + us) * 4, us, us, 0))
		{
		...
		{
			int cc = bp[targetY * bw + targetX];

			if (cc != pc)
			{
				floodFill(bp, bc, targetX, targetY, cc, pc);
			}

			gameState = GAMESTATE_TARGET;
			break;
		case GAMESTATE_DROPPER:
			pc = bp[targetY * bw + targetX];
